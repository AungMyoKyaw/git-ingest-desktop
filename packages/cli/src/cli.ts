...
import { generateProjectOutput, toGitIngestError, type OutputFormat, type ScanProjectOptions } from '@git-ingest/core'
...
export function parseArgs(argv: string[]): CliOptions {
  ...
  if (!options.rootDir) {
    throw new Error('Usage: git-ingest <folder> [--format markdown|text] [--output file] [--max-size bytes] [--include glob] [--exclude glob] [--copy]')
  }
