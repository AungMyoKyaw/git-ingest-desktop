...
import { isDefaultIgnoredDirectory, matchesPattern, normalizePatterns, readGitIgnorePatterns } from './ignore.js'
...
function throwIfAborted(signal?: AbortSignal) {
  if (signal?.aborted) {
    throw new AppError('CANCELLED', 'Generation was cancelled.', 'The active scan was cancelled before completion.')
  }
}
...
function buildWarnings(totalFiles: number, estimatedOutputBytes: number) {
  ...
  if (estimatedOutputBytes > 500_000) {
    warnings.push('The generated output will be large. Consider tighter excludes or a lower max file size.')
  }
...
async function validateRootDirectory(rootDir: string) {
  if (!rootDir || typeof rootDir !== 'string') {
    throw new AppError('INVALID_DIRECTORY', 'Choose a real project folder before scanning.', 'rootDir must be a string')
  }
  ...
  try {
    ...
    if (!details.isDirectory()) {
      throw new AppError('INVALID_DIRECTORY', 'Choose a folder instead of a file.', `${rootDir} is not a directory`)
    }
  } catch (error) {
    ...
    throw new AppError('INVALID_DIRECTORY', 'Choose a real project folder before scanning.', String(error))
  }
}
...
async function collectProject(options: ScanProjectOptions | LegacyScanProjectOptions, includeContent: boolean): Promise<InternalSummary> {
  ...
  async function walk(directory: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      emit(options.onProgress, { phase: 'processing', processedFiles: includedFiles.length, totalFiles })
      throwIfAborted(options.signal)
    }
  }
...
function toLegacyPreview(summary: ProjectSummary): LegacyPreviewResult {
  const includedEntries = summary.includedFiles.map((file) => ({ path: file.relativePath, ignored: false }))
  const skippedEntries = summary.skippedFiles.map((file) => ({ path: file.relativePath, ignored: true, reason: file.reason }))
  const ignoredDirectoryEntries = summary.ignoredDirectories.map((directory) => ({ path: directory.path, ignored: true, reason: directory.reason }))
...
export async function previewProject(options: LegacyScanProjectOptions): Promise<LegacyPreviewResult>
export async function previewProject(options: ScanProjectOptions | LegacyScanProjectOptions): Promise<ProjectSummary | LegacyPreviewResult> {
...
export async function generateProjectOutput(options: ScanProjectOptions | LegacyScanProjectOptions): Promise<GenerateProjectResult> {
  emit(options.onProgress, { phase: 'validating' })
  throwIfAborted(options.signal)
  const format: OutputFormat = options.format ?? 'markdown'
  const summary = await collectProject(options, true)
  throwIfAborted(options.signal)
  emit(options.onProgress, { phase: 'writing', processedFiles: summary.includedFiles.length, totalFiles: summary.totalFiles })
  throwIfAborted(options.signal)
