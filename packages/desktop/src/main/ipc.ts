...
import { loadAppState, rememberRecentProject, updateAppState } from './state.js'
import { validateExternalUrl, validateGeneratePayload, validatePreviewPayload } from './validators.js'
...
export function registerIpcHandlers(options: RegisterIpcHandlersOptions) {
  ...
  const generation = createGenerationManager({
    ...
    onFinished: async (message) => {
      if (message.status === 'success') {
        await updateAppState(userDataPath, (state) => rememberRecentProject(state, message.result.rootDir))
  ...
  ipcMain.handle('desktop:cancel-generation', async (event, payload) => {
    ...
    if (typeof payload?.requestId !== 'string' || payload.requestId.length === 0) {
      return { ok: false, error: { code: 'INVALID_REQUEST', userMessage: 'There is no active generation to cancel.', detail: null } }
    }
  ...
  ipcMain.handle('desktop:copy-output', async (event, payload) => {
    ...
    if (typeof payload?.output !== 'string' || payload.output.length === 0) {
      return { ok: false, error: { code: 'INVALID_OUTPUT', userMessage: 'Generate output before copying it.', detail: null } }
    }
  ...
  ipcMain.handle('desktop:save-output', async (event, payload) => {
    ...
    if (typeof payload?.output !== 'string' || payload.output.length === 0) {
      return { ok: false, error: { code: 'INVALID_OUTPUT', userMessage: 'Generate output before saving it.', detail: null } }
    }

    const format = payload?.format === 'text' ? 'text' : 'markdown'
    const projectName = typeof payload?.projectName === 'string' && payload.projectName.trim() ? payload.projectName : 'git-ingest'
  ...
  ipcMain.handle('desktop:open-output-file', async (event, payload) => {
    ...
    try {
      ...
      if (error) {
        return { ok: false, error: { code: 'OPEN_FAILED', userMessage: 'The file could not be opened.', detail: error } }
      }
