export type DropSelection =
  ...
  | { kind: 'error'; code: 'EMPTY_DROP' | 'MULTI_DROP' | 'FILE_DROP' | 'DROP_UNSUPPORTED'; message: string }
...
export function resolveDropSelection(input: DropSelectionInput): DropSelection {
  ...
  if (!dropped.path) {
    return { kind: 'error', code: 'DROP_UNSUPPORTED', message: 'Use Choose Folder if this drop target cannot provide a folder path.' }
  }
