  ...
  interface Window {
    gitIngest: {
      ...
      removeRecentProject: (
        ...
        | { ok: true; recentProjects: AppState['recentProjects'] }
        | { ok: false; error: DesktopError }
      >;
      preview: (
        payload: unknown,
      ) => Promise<{ ok: true; result: PreviewResult } | { ok: false; error: DesktopError }>;
      generate: (
        payload: unknown,
      ) => Promise<{ ok: true; requestId: string } | { ok: false; error: DesktopError }>;
      ...
      saveOutput: (payload: {
        ...
        projectName: string;
        format: 'markdown' | 'text';
      }) => Promise<{ ok: boolean; canceled?: boolean; filePath?: string; error?: DesktopError }>;
