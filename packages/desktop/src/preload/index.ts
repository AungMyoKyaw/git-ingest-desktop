...
contextBridge.exposeInMainWorld('gitIngest', {
  ...
  cancelGeneration: (requestId: string) => ipcRenderer.invoke('desktop:cancel-generation', { requestId }),
  copyOutput: (output: string) => ipcRenderer.invoke('desktop:copy-output', { output }),
  saveOutput: (payload: { output: string; projectName: string; format: 'markdown' | 'text' }) => ipcRenderer.invoke('desktop:save-output', payload),
  openOutputFile: (filePath: string) => ipcRenderer.invoke('desktop:open-output-file', { filePath }),
  revealOutputFile: (filePath: string) => ipcRenderer.invoke('desktop:reveal-output-file', { filePath }),
